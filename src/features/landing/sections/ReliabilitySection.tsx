        ...
        <section className="relative w-full py-24 bg-carbon">
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto px-6 sm:px-8 lg:pl-32">
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6 mb-16">
                    {features.map((item, i) => (
                        <div key={i} className="relative p-6 sm:p-8 bg-carbon border border-defense/10 hover:border-defense/40 transition-all duration-500 group overflow-hidden">
                            {/* Corner Accent */}
                            ...
                            <div className="relative z-10">
                                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-defense/5 border border-defense/15 flex items-center justify-center text-defense mb-6 group-hover:bg-defense/10 group-hover:border-defense/40 group-hover:shadow-[0_0_15px_rgba(255,0,0,0.15)] transition-all duration-500">
                                    <item.icon size={20} className="sm:w-6 sm:h-6" strokeWidth={1.5} />
                                </div>
                                <h3 className="text-lg sm:text-xl font-bold text-white/90 uppercase mb-3 group-hover:text-white transition-colors">{item.title}</h3>
                                <div className="h-[1px] w-8 bg-defense/30 group-hover:w-14 group-hover:bg-defense/60 transition-all duration-500 mb-2" />
                                <p className="text-gray-400 text-xs sm:text-sm leading-relaxed group-hover:text-gray-300 transition-colors">{item.desc}</p>
                            </div>
                ...
                <div className="w-full bg-[#120202] border border-defense-glow/30 flex flex-col md:flex-row items-stretch relative overflow-hidden group min-h-[100px]">
                    ...
                    <div className="w-full md:w-auto bg-[#1F0505] border-b md:border-b-0 md:border-r border-defense-glow/30 flex flex-col justify-center px-6 sm:px-8 py-4 sm:py-6 relative shrink-0">
                        <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-defense" />
                        <span className="text-defense font-mono font-bold text-[10px] sm:text-xs tracking-[0.2em] leading-relaxed block">
                            LIABILITY<br className="hidden sm:block" /> FORMULA
                        </span>
                        <div className="w-12 h-[1px] bg-defense-glow/50 mt-3 hidden sm:block" />
                    </div>
                    <div className="flex-1 flex items-center overflow-hidden py-4 sm:py-6 px-4 sm:px-6 md:px-12 relative">
                        <div className="w-full overflow-x-auto no-scrollbar py-2">
                            <p className="font-rajdhani font-medium text-white text-xs sm:text-sm md:text-base lg:text-lg tracking-wide whitespace-nowrap lg:whitespace-normal text-center lg:text-left">
                                {t.formula}
                            </p>
                        </div>
                    </div>
